...
| Phase 8 | Started with 8A field contract | Draft scope is branch-aware field propagation and mode overlap, improved oblique astigmatic q/matrix physics, focused CAD/prism assembly helpers, UI architecture hardening, and examples/docs/validators for each landed slice. The first 8A slice adds `KrakenOS.BranchField`, scalar paraxial propagation, Gaussian mode overlap, a runnable example, and `python -m KrakenOS.UI.validate_phase8_complete`. See `KRAKEN_UI_PHASE8_PLAN.md`. |
...
| P | Phase 8 field propagation and hardening | Started with branch-field contract and scalar propagation validator | High | High |
