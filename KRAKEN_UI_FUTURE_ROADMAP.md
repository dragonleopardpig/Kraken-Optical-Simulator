...
Historical Phase 5 manual cross-check notes have been folded back into the
coverage audit and roadmap instead of being rendered as a separate Sphinx page.
...
| Phase 8 | Started with 8A field UI; 8B complete at q-contract scope; 8D extraction underway | Draft scope is branch-aware field propagation and mode overlap, improved oblique astigmatic q/matrix physics, focused CAD/prism assembly helpers, UI architecture hardening, and examples/docs/validators for each landed slice. The first 8A slices add `KrakenOS.BranchField`, scalar paraxial propagation, Gaussian mode overlap, the UI `BField` intensity/phase/TEM00-overlap analysis with propagation distance and CSV export. Phase 8B adds `validate_phase8b_complete`, `validate_oblique_astigmatic_q`, `validate_branch_gaussian_q_report`, `Examp_Oblique_Astigmatic_Q.py`, `Actions -> Branch Gaussian Q Report`, first-order oblique spherical-refraction tangential/sagittal C terms, flat tilted-plate/TIR diagnostics, and real traced-layout validation on the `Galvo F-Theta Laser Scanner` preset. Phase 8D now extracts Branch Gaussian q report data/formatting into `KrakenOS/UI/branch_gaussian_q_report.py`, path-throughput aggregation/filter/report/export helpers into `KrakenOS/UI/branch_throughput_analysis.py`, coherent/diffraction detector math/export helpers into `KrakenOS/UI/coherent_detector_analysis.py`, branch-field promotion/CSV helpers into `KrakenOS/UI/branch_field_analysis.py`, detector map/path PSF/path MTF data/CSV helpers into `KrakenOS/UI/detector_path_analysis.py`, source-illumination record/sample/map/report helpers into `KrakenOS/UI/source_illumination_analysis.py`, scene-source normalization/source-object/summary helpers into `KrakenOS/UI/scene_source_analysis.py`, CAD/STL face-role metadata, world transforms, virtual splitter-plane crossings, hit-sequence helpers, face-fit pose solving, and snap-to-ray anchor scoring into `KrakenOS/UI/optical_solid_metadata.py`, STL diagnostics/transform helpers into `KrakenOS/UI/stl_geometry.py`, and CAD import/cache helpers into `KrakenOS/UI/cad_import_service.py` with service/UI parity validation. Embedded 3D plain left click now selects without rotating; left hold-drag rotates around the fixed current view focal point at constant sensitivity, and `Ctrl` + left drag follows the same path for compatibility; selected imported STEP overlays open a click-near rotation handler with persistent `X/Y/Z +/-90` buttons, the duplicate toolbar `STEP Rotate` menu was removed, active 3D workflow badges identify armed pick modes for `Center STEP Axis`, `Obj->LED`, `Center Row->Ray`, and `Source Target`, selected CAD/STL rows open a contextual placement handler instead of a persistent second toolbar, and Case Study 14 documents these 3D workflows with generated screenshots. Full thick tilted-plate wave propagation is deferred beyond the 8B q-contract scope. See `KRAKEN_UI_PHASE8_PLAN.md`. |

Phase 8D also adds `python -m KrakenOS.UI.validate_demo_readiness --full` as a
presentation guardrail: it runs the embedded 3D interaction contract, case-study
validators, menu smoke coverage, and Sphinx documentation build. The lower-level
`python -m KrakenOS.UI.validate_menu_smoke` still headlessly loads UI-menu
Layouts, Machine Vision layouts, and table-loadable Examples, then checks both
2-D rendering and
offscreen 3-D actor creation. Run it with `--include-zemax` before demos to
include testing Zemax prescriptions. Script-only and import-time file-writing
tutorials remain documented repository examples, not UI menu items.
...
| P | Phase 8 field propagation and hardening | Started with branch-field UI/export and first oblique-q refraction validator | High | High |
...
- Phase 7E adds worst-sample comparison, stack-up dashboard, worst-sample
  ...
  eligibility metadata, coupled tolerance sampling groups, named manufacturing
  metadata/templates, saved solve presets, covariance-aware stack-up bars, and
...
1. Add compensator eligibility presets if repeated solve-role patterns become
   common.
2. Refine tolerance dashboards interactively after user feedback.
