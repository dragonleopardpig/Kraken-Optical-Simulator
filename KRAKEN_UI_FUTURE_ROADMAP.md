...
| Phase 8 | Started with 8A field UI and 8B oblique-q physics | Draft scope is branch-aware field propagation and mode overlap, improved oblique astigmatic q/matrix physics, focused CAD/prism assembly helpers, UI architecture hardening, and examples/docs/validators for each landed slice. The first 8A slices add `KrakenOS.BranchField`, scalar paraxial propagation, Gaussian mode overlap, the UI `BField` intensity/phase/TEM00-overlap analysis with propagation distance and CSV export. The first 8B slices add `validate_oblique_astigmatic_q`, `validate_branch_gaussian_q_report`, `Examp_Oblique_Astigmatic_Q.py`, `Actions -> Branch Gaussian Q Report`, first-order oblique spherical-refraction tangential/sagittal C terms, flat tilted-plate/TIR diagnostics, and real traced-layout validation on the `Galvo F-Theta Laser Scanner` preset. See `KRAKEN_UI_PHASE8_PLAN.md`. |
