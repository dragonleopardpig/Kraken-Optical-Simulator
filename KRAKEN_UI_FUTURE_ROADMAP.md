...
| Phase 8 | Started with 8A field contract and UI field analysis | Draft scope is branch-aware field propagation and mode overlap, improved oblique astigmatic q/matrix physics, focused CAD/prism assembly helpers, UI architecture hardening, and examples/docs/validators for each landed slice. The first 8A slices add `KrakenOS.BranchField`, scalar paraxial propagation, Gaussian mode overlap, the UI `BField` intensity/phase/TEM00-overlap analysis with propagation distance and CSV export, a runnable example, and `python -m KrakenOS.UI.validate_phase8_complete`. See `KRAKEN_UI_PHASE8_PLAN.md`. |
...
| P | Phase 8 field propagation and hardening | Started with branch-field contract, scalar propagation validator, and UI `BField` analysis/export | High | High |
