...
| Phase 4 | In progress | 3D ray drawing now consumes shared `SceneBundle.ray_paths`, and embedded/legacy 3D share the optical surface mesh enumeration helper. Remaining work is full surface-mesh scene unification, native optimization-variable bridge, and removal of redundant special-case display code. |
...
`SceneBundle.ray_paths` as the 2D display path, and both 3D viewers share one
optical surface mesh enumeration helper.
