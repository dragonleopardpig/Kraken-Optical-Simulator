...
| Phase 8 | Started with 8A field UI and 8B oblique-q baseline | Draft scope is branch-aware field propagation and mode overlap, improved oblique astigmatic q/matrix physics, focused CAD/prism assembly helpers, UI architecture hardening, and examples/docs/validators for each landed slice. The first 8A slices add `KrakenOS.BranchField`, scalar paraxial propagation, Gaussian mode overlap, the UI `BField` intensity/phase/TEM00-overlap analysis with propagation distance and CSV export. The first 8B slice adds `validate_oblique_astigmatic_q` and `Examp_Oblique_Astigmatic_Q.py` to lock down current oblique mirror/refraction fallback contracts. See `KRAKEN_UI_PHASE8_PLAN.md`. |
...
| P | Phase 8 field propagation and hardening | Started with branch-field UI/export and oblique-q fallback validator | High | High |
