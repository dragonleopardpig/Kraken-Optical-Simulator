...
| Surface error maps | `Error_map = [X, Y, Z, SPACE]` | First-class at Phase 2 measured-map scope plus Phase 7E tolerance-overlay/compensator scope | Error-map import/reporting is first-class. Phase 7E adds deterministic tolerance Monte Carlo using marked optimization/native variables, tolerance-only vs compensator eligibility metadata, saved solve presets, worst-sample comparison, worst-sample compensator sweeps, multi-compensator coordinate solve, nominal-vs-worst spot/MTF/WFE overlays, and CSV export. Production stack-up dashboards and richer manufacturing constraints remain future work. |
