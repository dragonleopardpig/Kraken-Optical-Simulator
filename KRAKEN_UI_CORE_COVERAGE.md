...
| Surface error maps | `Error_map = [X, Y, Z, SPACE]` | First-class at Phase 2 measured-map scope plus Phase 7E tolerance-overlay/compensator scope | Error-map import/reporting is first-class. Phase 7E adds deterministic tolerance Monte Carlo using marked optimization/native variables, tolerance-only vs compensator eligibility metadata, `ToleranceCoupling` shared manufacturing groups, `ToleranceManufacturing` named source/type/tag/note metadata, saved solve presets, worst-sample comparison, stack-up dashboard with covariance-aware manufacturing-group bars, worst-sample compensator sweeps, multi-compensator coordinate solve, nominal-vs-worst spot/MTF/WFE overlays, and CSV export. Optional reusable manufacturing source templates remain future workflow polish. |
