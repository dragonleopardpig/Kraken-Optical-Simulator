...
- **Main remaining gap:** finish replacing legacy scalar `PrevN` mirrors at the remaining compatibility boundaries, then finish lifting folded preview curves, projected labels, and object/detector semantics behind the canonical scene/event table.
...
| 3D scene with 2D projections | Improving | `█████████░ 96%` | Reference planes, physical source markers, and display overlay curves now enter the scene as `(X, Y, Z)` geometry before YZ/XZ/XY projection. |
...
- Folded preview curves and projected labels are still pre-flattened to YZ display coordinates; XZ/XY plots therefore still rely on ray paths and mesh outlines for those legacy folded/annotation categories until they become fully 3D scene geometry.
