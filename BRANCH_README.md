...
Latest movement on 2026-05-26: optical-solid STL geometry helpers moved out of
`layout_editor.py` into `services/optical_solid_geometry.py`, and pure
ray-display geometry helpers moved into `services/ray_display_geometry.py`.
Low-level CAD/STEP export geometry helpers now live in
`services/cad_step_export.py`, and measured error-map parsing now lives in
`services/error_map_metadata.py`. Beam-splitter and diffuse/BRDF scatter
metadata normalization now lives in `services/beam_scatter_metadata.py`.
...
in `services/advanced_surface_validation.py`, and source/pupil/Gaussian/
scene-source modeling now lives in `services/source_modeling.py`. Together
these remove more than 4,600 source lines of non-Tk CAD/STL, ray-envelope,
STEP-export, surface-metrology parsing, optical-interaction metadata, and
source-launch modeling logic from the editor coordinator while keeping
backward-compatible symbols available from `layout_editor.py` for existing
validators and panels. The main editor coordinator is now 42,364 lines before
