...
| Modular production maintainability | In progress | `█████████░ 99%` | The main `layout_editor.py` file has been reduced to a thin Tk coordinator, with most operational behavior now owned by panels, widgets, services, validators, and focused helper modules. The ttk theme is now applied through a reusable adapter with optional `sv-ttk` support, public `ui` install metadata/runtime dependencies are validated, table-cell/menu/toolbar controls use reusable widget helpers, and source-structure budgets now guard the split. Remaining production-readiness work is concentrated in measured large-CAD performance budgets. |
