...
- `Snap STEP Normal->Optical Axis` now uses an entrance-face convention: the
  ...
  outward normal is aligned opposite the axis propagation direction. This keeps
  the imported solid upstream-facing after snap, so a subsequent Uncoated
...
- The Open 3D right panel is now a STEP element browser. It groups imported
  ...
  with Carry, Accept, Promote, Delete, Faces, Center Axis, and Normal->Axis
  actions scoped to the browser selection.
...
| --- | --- | --- | --- |
| `services/` boundary for Open 3D trace refresh | Started | `███████░░░ 72%` | `Open3DTraceRefreshService` owns sampling-mode normalization, Live Mode preview-bundle creation, open-inspector synchronization, and transient STEP live-trace row creation through the editor trace contract. The `open3d_face_pick` service owns through-body transparent CAD face picking for internal planes, and `Open3DStepStateService` owns the first delete-target state transition. Remaining service work is stale-request cancellation, CAD mesh reuse/throttling, and moving STEP overlay/promotion/face-assignment transitions behind one service-owned state machine. |
...
cannot diverge from the traced physics state. The next validator should drive
the same state machine through import, carry, snap, face assignment,
promotion, reassignment, and Trace Ray without relying on direct widget state
inside `Kraken3DInspector`.
