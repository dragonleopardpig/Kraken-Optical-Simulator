...
| Modular production maintainability | In progress | `█████████░ 95%` | The main `layout_editor.py` file has been reduced to a thin Tk coordinator, with most operational behavior now owned by panels, widgets, services, validators, and focused helper modules. The ttk theme is now applied through a reusable adapter with optional `sv-ttk` support, and public `ui` install metadata is validated in the fast contract runner. Remaining production-readiness work is concentrated in reusable menus/table-cell widgets, import-boundary cleanup, clean-venv install validation, and measured large-CAD performance budgets. |
...
| `widgets/` reusable Tk controls | Started | `█████████░ 90%` | `KrakenOS/UI/widgets/tooltips.py` owns reusable compact Tk tooltips, `commit_bindings.py` owns shared Entry/Combobox commit gestures, and `commit_controls.py` now exposes `CommitEntry`, `CommitCombobox`, labeled grid helpers, packed toolbar helpers, checkbutton helpers, radiobutton helpers, and command-button helpers for panels that should not hand-wire label/grid/pack/FocusOut/Return/KP_Enter/press-history behavior. `layout_shell_controls.py` uses the shared binding helper; Open 3D Live Controls, the main Source/Field/Trace/Display controls, the main table toolbar, the 2D projection selector controls, and the debug/projection/ray toolbar rows now use reusable widget helpers; and `validate_widget_commit_bindings` is included in the fast contract runner. Remaining reusable menus and table cell widgets remain the next reusable-widget extraction targets. |
