...
- Non-sequential `Pupil / field` layouts with a nonzero Source cone launch an
  actual 3D angular-pupil cone in Open 3D. `Meridional fan`, `Fan X`, and
  `Fan Y` remain 2D plot/analysis labels; the scene trace maps them to the
  default filled 3D angular pupil so Open 3D does not become a flat slice.
