...
| `widgets/` reusable Tk controls | Started | `██░░░░░░░░ 20%` | `KrakenOS/UI/widgets/tooltips.py` owns reusable compact Tk tooltips, and `commit_bindings.py` now owns shared Entry/Combobox commit gestures for FocusOut, Return, keypad Enter, and ComboboxSelected. `layout_shell_controls.py` routes deferred-refresh and manual-update entry bindings through the widget helper, with `validate_widget_commit_bindings` included in the fast contract runner. Validated entries, projection selectors, reusable menus, and table cell widgets still remain the next reusable-widget extraction targets. |
