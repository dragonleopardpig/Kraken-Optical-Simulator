...
- Open 3D imported STEP overlays can be carried with snapped movement while
  ...
  on that center. Dragging moves the center grip and component together in
  ...
  motion back into the drag loop and make the component jump unpredictably.
  Hold `Ctrl` while left-dragging to rotate the 3D view; press `Esc` to cancel
  ...
  `Carry -> Snap step` selector switches Auto/Fine/Coarse movement. `Snap ray`
