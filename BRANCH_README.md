...
| Oblique astigmatic Gaussian q | Phase 8B started | `python -m KrakenOS.UI.validate_oblique_astigmatic_q` and `KrakenOS/Examples/Examp_Oblique_Astigmatic_Q.py` now lock down flat-fold, oblique spherical mirror, near-normal refraction, first-order oblique spherical-refraction tangential/sagittal C terms, and a real traced `Galvo F-Theta Laser Scanner` UI layout with oblique refractive hits. |
...
Practical recommendation: continue Phase 8B by adding explicit finite tilted
plate and TIR/unsupported-case diagnostics now that the oblique q update is
validated against a real traced UI layout. Richer mode targets for `BField` can
follow after that. The detector-bin Gaussian-q recombination path now exists,
