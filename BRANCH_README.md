...
Latest movement on 2026-05-23: CAD/STL face-function assignment now invalidates
the stale traced system, raykeeper, SceneBundle, and transient STEP trace-plan
caches as one physics-edit operation. The Open 3D face-role Save Roles path also
forces an already-open inspector to retrace, preventing a newly assigned
`Full Reflecting` face from continuing to display the prior `F004 refraction`
bundle.
