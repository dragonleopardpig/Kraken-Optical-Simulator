...
| 3D scene with 2D projections | Achieved | `██████████ 100%` | YZ, XZ, and XY views are generated from traced 3D scene data; Open 3D uses the same world trace envelope, categorized view/scene control rows with a toolbar layout validator, direct STEP import, snapped cube-grid STEP carry placement with Sphinx coverage, in-scene STEP rotation handles, status-aware detector-miss terminal markers, active detector footprints, miss crosshairs, hover/click terminal diagnostics, and row-sized Object/Image reference display. |
...
- Open 3D imported STEP overlays can be carried on an automatically sized
  ...
  Open 3D screenshot of this carry workflow.
