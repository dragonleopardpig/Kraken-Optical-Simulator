...
- Ray display filters show all rays, detector hits, missed detector paths,
  ...
  Open 3D bounds-caps display-only escaped ray segments before adding VTK
  actors, and caps missed-detector display diagnostics inside the detector
  plane instead of moving the endpoint to an arbitrary off-plane location. The
