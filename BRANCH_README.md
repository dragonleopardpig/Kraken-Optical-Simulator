...
- `Actions -> 3D Place/Orient Selected CAD/STL Solid` opens the current 3D view
  ...
  fallback. Embedded 3D now opens a contextual `CAD/STL placement handler`
  popup instead of a persistent second-row toolbar. Users can rotate the mesh
  while watching it in 3D, fit mesh-local
...
| Phase 8D UI hardening | Started | Branch Gaussian q report collection, summary/report formatting, table values, and CSV columns are extracted into `KrakenOS/UI/branch_gaussian_q_report.py`. Path-throughput aggregation, path filter choices/matching, report text/table values, branch path labels, and CSV export are extracted into `KrakenOS/UI/branch_throughput_analysis.py`. Coherent detector constants, grouping/pair keys, coherent CSV row export, FFT vector-field intensity, and diffraction angular-spectrum data are extracted into `KrakenOS/UI/coherent_detector_analysis.py`. Branch-field detector promotion, scalar propagation, TEM00 overlap data, and CSV export are extracted into `KrakenOS/UI/branch_field_analysis.py`. Detector map, path PSF, and path MTF data assembly, histogram/FFT calculations, CSV schemas, and row generation are extracted into `KrakenOS/UI/detector_path_analysis.py`. Source-illumination record grouping, target-hit sample assembly, map extent/binning/density, per-source centroid calculations, report formatting, and CSV export are extracted into `KrakenOS/UI/source_illumination_analysis.py`. Scene-source spec normalization/deduping, setting serialization, source-object construction, visible source feature/detail text, and Source panel/Scene Source Manager summaries are extracted into `KrakenOS/UI/scene_source_analysis.py`. CAD/STL optical-solid face-role metadata normalization, side/function coercion, auto side-label assignment, virtual cube-splitter plane construction, colors, summary text, world transforms, face matching, virtual-plane crossings, hit-sequence classification, face-fit pose solving, auto-roll reference selection, and snap-to-ray anchor scoring are extracted into `KrakenOS/UI/optical_solid_metadata.py`. STL byte parsing, mesh diagnostics/formatting, transformed bounds/points, and reusable 2-D hull helpers are extracted into `KrakenOS/UI/stl_geometry.py`. CAD cache-path construction, STEP/IGES-to-STL conversion, source-to-mesh resolution, and external CAD helper-tool subprocess wrappers are extracted into `KrakenOS/UI/cad_import_service.py`. Embedded 3D plain left click selects without rotating the camera; left hold-drag rotates around the current view focal point with fixed sensitivity and no inertial acceleration; `Ctrl` + left drag follows the same path for compatibility; selected imported STEP overlays open a click-near rotation handler with persistent `X/Y/Z +/-90` buttons, the duplicate toolbar `STEP Rotate` menu was removed, active 3D workflow badges identify armed pick modes for `Center STEP Axis`, `Obj->LED`, `Center Row->Ray`, and `Source Target`, and selected CAD/STL rows open a contextual placement handler instead of a persistent second toolbar. `layout_editor.py` retains Tk dialogs, plotting, progress, trace collection, table synchronization, STL face clustering, CAD/STEP overlay state, and compatibility wrappers, while `validate_branch_gaussian_q_report`, `validate_diffraction_detector`, `validate_detector_sampling_stability`, `validate_phase8_field_contract`, `validate_branch_analysis`, `validate_scene_row_mapping`, `validate_optical_cad_solid_import`, `validate_optical_solid_face_roles`, `validate_optical_solid_face_fit`, `validate_optical_solid_virtual_plane`, `validate_optical_solid_hit_sequence`, `validate_multi_scene_sources`, and `validate_demo_readiness` check service/UI parity, export/map contracts, and presentation readiness. |
...
renderer, and checked for offscreen 3-D actors. `--include-zemax` extends the
same check to the testing Zemax prescription menu. Script-only examples remain
runnable/documented, but are excluded from the UI menu because they are not
table-loadable optical systems; import-time file-writing tutorials are also kept
out of the UI menu.
