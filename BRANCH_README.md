...
lives in `services/optical_solid_workflow.py`. The main editor coordinator is
now about 7,965 lines, while validators continue to access the same inherited
...
left/control-panel coordination, plot hover/viewer helpers, and analysis-display
glue out of `layout_editor.py`. The next large slice should move the main
left-sidebar/control synchronization and trace-mode UI state into a focused
application-shell/control mixin, then replace transitional late-bound sync
points with dedicated constants/helper modules. Keep `layout_editor.py` as the
