...
| `panels/` boundary for Open 3D controls | Started | `█████████▎ 93%` | `Open3DLiveControlsPanel` owns the left-docked Live Controls UI, `Open3DTopControlsPanel` owns the View, Scene, and Carry toolbar rows, and `MainSourceControlsPanel`, `MainFieldControlsPanel`, `MainTraceDisplayControlsPanel`, `MainAnalysisToolbarPanel`/`MainInformationPanel`, `MainOptimizationPanel`, `MainAtmospherePanel`, plus `MainCoatingMaterialDialog` own the main Source, Field, Trace/Display, analysis-toolbar, Information, Optimization, Atmosphere, and Coating/Material editor surfaces without moving analysis, optimization, or coating validation execution out of the editor model. `Kraken3DInspector` keeps thin compatibility callbacks. Remaining panel work is smaller specialized editor dialogs. |
...
1. Split `KrakenOS/UI/layout_editor.py` into a package-style structure inspired
   ...
   `KrakenOS/UI/panels/main_optimization_panel.py`, and
   `KrakenOS/UI/panels/main_atmosphere_panel.py`, and
   `KrakenOS/UI/panels/main_coating_material_dialog.py` now own the main
   Source, Field, Trace/Display, Analysis toolbar, Information table,
   Optimization, Atmosphere, and Coating/Material controls while delegating
   state and callbacks back to the editor.
