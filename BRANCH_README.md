...
| Live 3D authoring | Started | `███░░░░░░░ 30%` | Open 3D now has a docked Live Controls panel bound to the same Source, Field, and Trace / Display variables as the main left panel. Live Mode schedules debounced 3D retraces after source edits, main left-panel edits, and STEP carry/placement changes, using the same 3D preview sampling path. The remaining architecture work is making unpromoted imported STEP overlays transiently traceable during free drag, then promoting that transient path into a persistent row when the user accepts placement. |
...
- Open 3D has a docked `Live Controls` panel for the same Source, Field, and
  Trace / Display state that lives in the main left panel. `Live Mode` uses a
  debounced 3D retrace scheduler, so source changes and row-backed placement
  edits can update the ray scene without returning to the 2D editor. Manual
  `Trace now` uses the 3D sampling mode even when Live Mode is off.
