...
- Open 3D right-click on a CAD/STL optical face opens a compact function menu:
  ...
  `Uncoated` is stored as a physical interaction surface rather than an
  inferred output port, so reassignment cycles do not create hidden output-port
  anchors.
