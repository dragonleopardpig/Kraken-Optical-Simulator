...
| Modular production maintainability | In progress | `█████████░ 98%` | The main `layout_editor.py` file has been reduced to a thin Tk coordinator, with most operational behavior now owned by panels, widgets, services, validators, and focused helper modules. The ttk theme is now applied through a reusable adapter with optional `sv-ttk` support, public `ui` install metadata is validated in the fast contract runner, table-cell/menu/toolbar controls use reusable widget helpers, and source-structure budgets now guard the split. Remaining production-readiness work is concentrated in clean-venv install validation and measured large-CAD performance budgets. |
...
| Public `kraken-os[ui]` install path | In progress | `████████░░ 80%` | `pyproject.toml` and legacy `setup.py` now expose aligned `ui` and `optimization` extras that include optimizer backend dependencies plus `sv-ttk` for the themed Tk visual layer. `validate_ui_install_metadata` is in the fast contract runner so dependency drift is caught without a network install. A clean-venv install validation and final package-name normalization remain before this is marked complete. |
