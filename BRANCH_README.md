...
synchronization now lives in `services/layout_shell_controls.py`. The main
editor coordinator is now about 6,605 lines, while validators continue to
...
plot hover/viewer helpers, table editing leftovers, and analysis-display glue
out of `layout_editor.py`. The next large slice should move 2D plot hover,
selection-overlay, high-resolution viewer, and layout pick helpers into a
focused plot interaction mixin, then replace transitional late-bound sync points
with dedicated constants/helper modules. Keep `layout_editor.py` as the
application coordinator instead of a behavior warehouse.
