...
| Open 3D imported-solid resize & off-beam display-only | Complete (live drag-arrow handle pending) | `90% [#########.]` | An imported STEP solid can be resized in place from Open 3D (right-click body → **Resize Solid…**). A detected beam-splitter cube resizes with 2 DOF — a coupled square **cross-section** + a free **depth** — so its 45° coating stays at 45° (the real 50³ cube goes to 55×55×78 holding the coating at (0.707,0.707,0); an un-coupled 55×50×78 would tilt it). Any other solid resizes per-axis (W×H×D). The scale runs in **mesh space** (anchored vertex scale about the fixed face; a non-uniform `BRepBuilderAPI_GTransform` would degrade analytic Plane faces to BSpline), coupling is detected off the original B-rep planes, and promotion + face metadata inherit the resize automatically (the builders mesh the transformed overlay). The cube's **interior 45° coating is recovered as a selectable face** so the splitter coating can be assigned after promotion. Separately, a promoted solid dragged clear of the beam is **display-only**: `neutralize_offbeam_inert_solids` (at the top of the single `_build_system_from_specs` chokepoint) replaces each off-beam inert solid with a flat zero-power AIR surface, so it draws but contributes zero optical effect until on-beam or coated (bugs/0065 — fixes the off-beam cube that was corrupting the on-axis trace/image-circle). The pure drag-a-face **gesture planner** (face→axis/anchor/coupling → the popup's apply-ready tuple) is tested; the live VTK arrow handle that drives it is the remaining in-app step. Guards: `validate_open3d_solid_resize`, `…_overlay`, `…_beam_splitter_coating_recovered`, `…_resize_gesture`, `…_offbeam_solid_display_only` (penta Phases 66-70). |
