...
| Oblique astigmatic Gaussian q | Phase 8B started | `python -m KrakenOS.UI.validate_oblique_astigmatic_q` and `KrakenOS/Examples/Examp_Oblique_Astigmatic_Q.py` now lock down flat-fold, oblique spherical mirror, near-normal refraction, and first-order oblique spherical-refraction tangential/sagittal C terms. |
...
Practical recommendation: continue Phase 8B by validating the oblique q update
against a traced UI layout rather than only synthetic hits, then add finite
tilted plate diagnostics. Richer mode targets for `BField` can follow after
that. The detector-bin Gaussian-q recombination path now exists, but it is
