...
| Oblique astigmatic Gaussian q | Phase 8B baseline started | `python -m KrakenOS.UI.validate_oblique_astigmatic_q` and `KrakenOS/Examples/Examp_Oblique_Astigmatic_Q.py` now lock down current flat-fold, oblique spherical mirror, near-normal refraction, and explicit oblique-refraction fallback behavior before new matrix physics lands. |
...
Practical recommendation: continue Phase 8B by implementing one narrow oblique
q physics improvement against the new validator baseline, most likely the
tilted powered refractive-surface matrix or finite tilted plate diagnostics.
Richer mode targets for `BField` can follow after that. The detector-bin
