...
| `panels/` boundary for Open 3D controls | Started | `█████████▌ 95%` | `Open3DLiveControlsPanel` owns the left-docked Live Controls UI, `Open3DTopControlsPanel` owns the View, Scene, and Carry toolbar rows, and `MainSourceControlsPanel`, `MainFieldControlsPanel`, `MainTraceDisplayControlsPanel`, `MainAnalysisToolbarPanel`/`MainInformationPanel`, `MainOptimizationPanel`, `MainAtmospherePanel`, `MainCoatingMaterialDialog`, `MainDiffuseScatterDialog`, plus `MainSurfaceShapeBuilderDialog` own the main Source, Field, Trace/Display, analysis-toolbar, Information, Optimization, Atmosphere, Coating/Material, Diffuse/BRDF, and Surface Shape Builder editor surfaces without moving analysis, optimization, coating, scatter, or shape validation execution out of the editor model. `Kraken3DInspector` keeps thin compatibility callbacks. Remaining panel work is smaller specialized editor dialogs. |
...
1. Split `KrakenOS/UI/layout_editor.py` into a package-style structure inspired
   ...
   `KrakenOS/UI/panels/main_diffuse_scatter_dialog.py`, and
   `KrakenOS/UI/panels/main_surface_shape_builder_dialog.py` now own the main
   Source, Field, Trace/Display, Analysis toolbar, Information table,
   Optimization, Atmosphere, Coating/Material, Diffuse/BRDF, and Surface Shape
   Builder controls while delegating state and callbacks back to the editor.
