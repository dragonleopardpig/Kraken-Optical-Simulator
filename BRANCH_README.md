...
| Full field FFT / mode-overlap propagation | Phase 8 started | First slices add `KrakenOS.BranchField`, scalar paraxial propagation, Gaussian TEM00 mode-overlap, the UI `BField` intensity/phase/TEM00-overlap analysis, `KrakenOS/Examples/Examp_Branch_Field_Propagation.py`, and `python -m KrakenOS.UI.validate_phase8_complete`. |
...
Practical recommendation: continue Phase 8 by adding explicit propagation
distance/export controls to the `BField` workflow or by starting the oblique
astigmatic q/matrix slice. Full oblique astigmatic surface matrices should
follow once the branch-field UI contract is stable. The detector-bin Gaussian-q
