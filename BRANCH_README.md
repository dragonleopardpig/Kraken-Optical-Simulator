...
| Oblique astigmatic Gaussian q | Phase 8B started | `python -m KrakenOS.UI.validate_oblique_astigmatic_q` and `KrakenOS/Examples/Examp_Oblique_Astigmatic_Q.py` now lock down flat-fold, oblique spherical mirror, near-normal refraction, first-order oblique spherical-refraction tangential/sagittal C terms, flat tilted-plate q-only index-step diagnostics, TIR-deferred diagnostics, and a real traced `Galvo F-Theta Laser Scanner` UI layout with oblique refractive hits. |
