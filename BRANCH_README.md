...
| `widgets/` reusable Tk controls | Started | `████████░░ 75%` | `KrakenOS/UI/widgets/tooltips.py` owns reusable compact Tk tooltips, `commit_bindings.py` owns shared Entry/Combobox commit gestures, and `commit_controls.py` now exposes `CommitEntry`, `CommitCombobox`, `grid_labeled_commit_entry`, and `grid_labeled_commit_combobox` for panels that should not hand-wire label/grid/FocusOut/Return/KP_Enter behavior. `layout_shell_controls.py` uses the shared binding helper; Open 3D Live Controls plus the main Source, Field, and Trace/Display numeric controls now use reusable labeled rows; and `validate_widget_commit_bindings` is included in the fast contract runner. Projection selectors, remaining reusable menus, and table cell widgets remain the next reusable-widget extraction targets. |
