...
- Non-sequential `Pupil / field` layouts with a nonzero Source cone use the
  selected pupil pattern in Open 3D. `Meridional fan`, `Fan X`, and `Fan Y`
  launch deterministic source-point fans in the requested section plane, while
  azimuthal patterns such as `Hexapolar`, `Square`, and `Random disk` launch the
  deterministic 3D cone needed for full-envelope prism/CAD inspection.
