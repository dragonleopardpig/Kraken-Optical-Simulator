...
- Ray display filters show all rays, detector hits, missed detector paths,
  ...
  The shared 2D/Open 3D display path bounds-caps escaped ray tails before
  autoscale/rendering, and caps missed-detector display diagnostics inside the
  detector plane instead of moving the endpoint to an arbitrary off-plane
  location. The canonical ray path, event metadata, raykeeper data, and CSV
