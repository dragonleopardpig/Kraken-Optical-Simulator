...
| `widgets/` reusable Tk controls | Started | `██████░░░░ 60%` | `KrakenOS/UI/widgets/tooltips.py` owns reusable compact Tk tooltips, `commit_bindings.py` owns shared Entry/Combobox commit gestures, and `commit_controls.py` now exposes `CommitEntry` plus `grid_labeled_commit_entry` for panels that should not hand-wire label/grid/FocusOut/Return/KP_Enter behavior. `layout_shell_controls.py` uses the shared binding helper; Open 3D Live Controls, main Field controls, and Trace/Display numeric controls now use the reusable labeled-entry row; and `validate_widget_commit_bindings` is included in the fast contract runner. Source numeric entries, projection selectors, reusable menus, and table cell widgets remain the next reusable-widget extraction targets. |
