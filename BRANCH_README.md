...
- Open 3D imported STEP overlays are carried with free movement. Imported STEP
  ...
  assignment remains available on the selected row. Assigned optical faces are
  tinted in the 3D scene so previously authored Uncoated, Reflective, splitter,
  and absorber surfaces remain visible before the next pick. The CAD/target
  promote menu turns the current placed overlay into a cached file-backed optical
  ...
  original STEP overlay and remains present after the trace scene rebuilds.
