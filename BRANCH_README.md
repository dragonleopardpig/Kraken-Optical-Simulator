...
| `panels/` boundary for Open 3D controls | Started | `████████▋░ 87%` | `Open3DLiveControlsPanel` owns the left-docked Live Controls UI, `Open3DTopControlsPanel` owns the View, Scene, and Carry toolbar rows, and `MainSourceControlsPanel`, `MainFieldControlsPanel`, `MainTraceDisplayControlsPanel`, plus `MainAnalysisToolbarPanel`/`MainInformationPanel` own the main Source, Field, Trace/Display, analysis-toolbar, and right Information surfaces without moving analysis execution out of the editor model. `Kraken3DInspector` keeps thin compatibility callbacks. Remaining panel work is extracting optimization and smaller dialog panels. |
...
1. Split `KrakenOS/UI/layout_editor.py` into a package-style structure inspired
   ...
   `KrakenOS/UI/panels/main_analysis_controls.py` now own the main Source,
   Field, Trace/Display, Analysis toolbar, and Information table controls while
