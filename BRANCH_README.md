...
| Phase 8D UI hardening | Started | Branch Gaussian q report collection, summary/report formatting, table values, and CSV columns are extracted into `KrakenOS/UI/branch_gaussian_q_report.py`. Coherent detector constants, grouping/pair keys, coherent CSV row export, FFT vector-field intensity, and diffraction angular-spectrum data are extracted into `KrakenOS/UI/coherent_detector_analysis.py`. `layout_editor.py` retains Tk dialogs, plotting, progress, trace collection, and compatibility wrappers, while `validate_branch_gaussian_q_report`, `validate_diffraction_detector`, and `validate_detector_sampling_stability` check service/UI parity and export contracts. |
