...
- Open 3D imported STEP overlays can be carried with snapped movement while
  ...
  the full traced 3D scene for every snapped step. Hold `Ctrl` while
  left-dragging to pause carry and rotate the 3D view. The component can be
  walked toward traced rays before orientation is adjusted with the colored
  handles. The 3D hardware-alignment Sphinx case study now includes a captured
  Open 3D screenshot of this carry workflow. A visible `Carry -> Snap step`
  ...
  a picked traced 3D ray point, `Snap target` places it on a detector, Object
  row, active target surface, or CAD/STL face anchor through the shared scene
