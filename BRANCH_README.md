...
- Open 3D imported STEP overlays are carried with free movement. Imported STEP
  ...
  click mode where only blue Optical Axis overlays are accepted; these overlays
  are generated from traced path branches and remain pickable even when regular
  rays are hidden. Click the intended `Optical Axis 1`, `Optical Axis 2`,
  `Optical Axis 3`, and so on. The picked face center moves onto the clicked
  optical path point, and the picked face normal is rotated parallel to that
