...
| Modular production maintainability | In progress | `█████████░ 97%` | The main `layout_editor.py` file has been reduced to a thin Tk coordinator, with most operational behavior now owned by panels, widgets, services, validators, and focused helper modules. The ttk theme is now applied through a reusable adapter with optional `sv-ttk` support, public `ui` install metadata is validated in the fast contract runner, and table-cell/menu/toolbar controls use reusable widget helpers instead of local ad hoc bindings. Remaining production-readiness work is concentrated in import-boundary cleanup, clean-venv install validation, and measured large-CAD performance budgets. |
