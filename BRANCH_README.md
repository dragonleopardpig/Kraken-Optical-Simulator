...
`services/cad_step_export.py`. Together these remove about 1,800 source lines
of non-Tk CAD/STL, ray-envelope, and STEP-export logic from the editor
coordinator while keeping backward-compatible symbols available from
`layout_editor.py` for existing validators and panels. The main editor
coordinator is now 45,432 lines before the next extraction slice.
