...
| `panels/` boundary for Open 3D controls | Started | `████████▍░ 84%` | `Open3DLiveControlsPanel` owns the left-docked Live Controls UI, `Open3DTopControlsPanel` owns the View, Scene, and Carry toolbar rows, and `MainSourceControlsPanel`, `MainFieldControlsPanel`, plus `MainTraceDisplayControlsPanel` own the main left-panel Source, Field, and Trace/Display controls without moving source, field, or trace policy out of the editor model. `Kraken3DInspector` keeps thin compatibility callbacks. Remaining panel work is extracting analysis, optimization, and smaller dialog panels. |
...
1. Split `KrakenOS/UI/layout_editor.py` into a package-style structure inspired
   ...
   `KrakenOS/UI/panels/main_trace_display_controls.py` now own the main left
   Source, Field, and Trace/Display controls while delegating state and
