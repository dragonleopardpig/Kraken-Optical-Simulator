...
- Non-sequential `Pupil / field` layouts with a nonzero Source cone launch a
  deterministic 3D cone from the source point. This covers prism/CAD scenes that
  still use the default source panel instead of an explicit physical source
  record, and prevents Open 3D from showing a flat display-plane fan as the
  scene truth.
...
- Open 3D placement rotation handles can rotate the selected surface row around
  ...
  per axis with sharp opposed cone arrowheads; clicking a handle applies a
