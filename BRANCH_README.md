...
Five invariants define the target architecture:
...
- Never hide KrakenOS-native state behind a UI-only abstraction without
  ...
  CSV export.
- Keep UI implementation details modular enough that a maintainer can trace a
  feature from widget to service to validator without reading unrelated CAD,
  plotting, source, optimization, or file-export code.
...
| Modular production maintainability | Achieved | `██████████ 100%` | The main `layout_editor.py` file has been reduced to a thin Tk coordinator, with most operational behavior now owned by panels, widgets, services, validators, and focused helper modules. The ttk theme is applied through a reusable adapter with optional `sv-ttk` support, public `ui` install metadata/runtime dependencies are validated, table-cell/menu/toolbar controls use reusable widget helpers, source-structure budgets guard the split, and Open 3D Live Mode debounce/cancellation/retry budgets are explicit validated contracts. |
...
checks that each saved native STEP body is placed in the trace/display frame
from the row-backed source-to-trace transform. `TRANS_2A` is intentionally not
applied a second time because `SYSTEM.AAA` optical-solid meshes are already in
the same coordinate frame as traced raykeeper points. The five-penta FreeCAD
detachment case now fails unless every exported prism body remains coincident
with the traced runtime mesh to within sub-millimetre tolerance after the STEP
file is written and read back again. The local
`attachment/five_penta_prism_cascade_3d.step` fixture has been regenerated from
that validated native export so FreeCAD/OpenCASCADE review uses the same
...
- The `ui` extra installs the optimizer runtime dependencies, including
  `pygmo`, and the Tk visual layer dependency `sv-ttk`. If the Optimization
  panel reports that the backend is unavailable, confirm the active environment
  with `python -m KrakenOS.UI.validate_optimization_backend`. To check the full
  UI extra, run `python -m KrakenOS.UI.validate_ui_install_runtime`.
...
| `widgets/` reusable Tk controls | Complete | `██████████ 100%` | `KrakenOS/UI/widgets/tooltips.py` owns reusable compact Tk tooltips, `commit_bindings.py` owns shared Entry/Combobox commit gestures, `commit_controls.py` exposes `CommitEntry`, `CommitCombobox`, labeled grid helpers, packed toolbar helpers, checkbutton helpers, radiobutton helpers, and command-button helpers, `table_cell_editor.py` owns inline Treeview cell editor placement with standard FocusOut/Return/KP_Enter/Escape behavior, and `menu_controls.py` owns compact popup/menu button builders. `layout_shell_controls.py` uses the shared binding helper; Open 3D Live Controls, the Open 3D top toolbar menus, the main Source/Field/Trace/Display controls, the main table toolbar, the 2D projection selector controls, the debug/projection/ray toolbar rows, and editable table cells now use reusable widget helpers; and `validate_widget_commit_bindings` is included in the fast contract runner. |
...
| Modular maintainability budget validator | Complete | `██████████ 100%` | `validate_ui_modular_maintainability` enforces line budgets for the coordinator and largest extracted modules, confirms panels/services/widgets remain split into dedicated modules, requires the reusable widget modules, checks the theme adapter is applied from the coordinator, and confirms key packaging/widget/toolbar contracts stay registered in the fast runner. |
| Live Mode performance service | Complete | `██████████ 100%` | `Open3DLiveRefreshService` owns debouncing, cancellation, busy/pending stale-request handling, bounded delay normalization, and delayed retry scheduling through named budget constants. `validate_open3d_live_performance_budget` simulates stale callback cancellation, busy pending-state coalescing, retry scheduling, delay clamping, and status-path behavior without opening VTK/Tk, so large-CAD Live Mode work has a maintained performance contract before any future mesh-reuse tuning. |
...
| `sv-ttk` theme adapter | Complete | `██████████ 100%` | `modern_ttk_theme.py` is now applied during editor startup. It uses `sv-ttk` when the UI extra installs it, records the active backend for diagnostics, keeps the Kraken palette/style overrides centralized, and falls back to the existing clam-based ttk styling when `sv-ttk` is unavailable or `KRAKEN_UI_TTK_THEME=classic/off` is requested. |
| Public `kraken-os[ui]` install path | Complete | `██████████ 100%` | `pyproject.toml` and legacy `setup.py` expose aligned `ui` and `optimization` extras that include optimizer backend dependencies plus `sv-ttk` for the themed Tk visual layer. `validate_ui_install_metadata` is in the fast contract runner so dependency drift is caught without a network install, and `validate_ui_install_runtime` checks an installed `.[ui]` environment. The documented regular-Python path now includes both the editable install and runtime validation command. |
...
4,053 lines, while `modern_ttk_theme.py` applies the optional `sv-ttk` visual
adapter at startup and `validate_ui_install_metadata` keeps public UI extras
aligned across packaging metadata. Validators continue to access the same inherited public
...
4. Keep the `sv-ttk` adapter as a thin visual layer. The split is now far
   enough along that `modern_ttk_theme.py` can initialize `sv-ttk` when the
   `ui` extra is installed, fall back to native ttk/clam when it is not, and
   leave physics/scene services untouched.

5. Maintain the public install story for the branch. The target remains
   `pip install kraken-os[ui]` from a normal Python environment. Before a
   packaged release exists, the documented bridge is:
   ...
   The packaging metadata now makes the UI and optimizer extras explicit, keeps
   CPU tracing as the reliable default, and provides
   `validate_ui_install_metadata` plus `validate_ui_install_runtime` to keep the
   editable-branch install path testable.
