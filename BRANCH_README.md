...
`layout_editor.py` into `services/optical_solid_geometry.py`. This pulls
704 lines of non-Tk face clustering, face metadata marker, face-fit, and STL
transform helpers into a focused service while keeping backward-compatible
symbols available from `layout_editor.py` for existing validators and panels.
The main editor coordinator is now 46,537 lines before the next extraction
slice.
