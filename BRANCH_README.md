...
- Non-sequential `Pupil / field` layouts with a nonzero Source cone launch from
  a 3D reference aperture/pupil in Open 3D and in saved layout scripts. A
  physical point-emitter cone is explicit: use `Random point cone` or a physical
  Scene Source Manager source. `Meridional fan`, `Fan X`, and `Fan Y` remain
  2D plot/analysis labels, not instructions to collapse the 3D scene into a
  flat slice or a one-point cone.
