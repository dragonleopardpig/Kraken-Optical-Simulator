...
| Live 3D authoring | In progress | `█████░░░░░ 50%` | Open 3D now has a docked Live Controls panel bound to the same Source, Field, and Trace / Display variables as the main left panel. Live Mode schedules debounced 3D retraces after source edits, main left-panel edits, and STEP carry/placement changes, using the same 3D preview sampling path. Imported arbitrary optical STEP overlays now enter live traces as transient file-backed optical solid rows, so rays can interact with the unpromoted overlay during placement without inserting a row into the editable table. The remaining architecture work is making that transient path fast enough for continuous drag on large CAD meshes and turning accepted placement into an explicit persistent row workflow. |
