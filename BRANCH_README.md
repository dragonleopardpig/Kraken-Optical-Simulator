...
| Modular production maintainability | In progress | `█████████░ 96%` | The main `layout_editor.py` file has been reduced to a thin Tk coordinator, with most operational behavior now owned by panels, widgets, services, validators, and focused helper modules. The ttk theme is now applied through a reusable adapter with optional `sv-ttk` support, public `ui` install metadata is validated in the fast contract runner, and inline table-cell editing uses a reusable widget helper instead of local ad hoc bindings. Remaining production-readiness work is concentrated in reusable menu builders, import-boundary cleanup, clean-venv install validation, and measured large-CAD performance budgets. |
...
| `widgets/` reusable Tk controls | In progress | `█████████░ 95%` | `KrakenOS/UI/widgets/tooltips.py` owns reusable compact Tk tooltips, `commit_bindings.py` owns shared Entry/Combobox commit gestures, `commit_controls.py` exposes `CommitEntry`, `CommitCombobox`, labeled grid helpers, packed toolbar helpers, checkbutton helpers, radiobutton helpers, and command-button helpers, and `table_cell_editor.py` now owns inline Treeview cell editor placement with standard FocusOut/Return/KP_Enter/Escape behavior. `layout_shell_controls.py` uses the shared binding helper; Open 3D Live Controls, the main Source/Field/Trace/Display controls, the main table toolbar, the 2D projection selector controls, the debug/projection/ray toolbar rows, and editable table cells now use reusable widget helpers; and `validate_widget_commit_bindings` is included in the fast contract runner. Remaining reusable menu/dropdown builders are the final reusable-widget extraction target. |
