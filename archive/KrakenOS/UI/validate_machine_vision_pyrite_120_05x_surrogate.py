"""Validate the PYRITE 5.6/120/0.5x machine-vision blackbox surrogate.

Schneider-Kreuznach PYRITE 5.6/120/0.5x V38 (ID 1097787). The surrogate is a
paraxial two-thin-lens blackbox solved to reproduce the public datasheet
cardinals (f'eff=119.75, SF=-95.36, S'F'=93.90, Sigma d=49.71) while overlaying
the bundled vendor STEP so the camera-glue / lens-datum glue / defocus / detector
features work out of the box.

Unlike the 1X / 85 mm sibling validators this guard does not require a docs
tutorial page or a rendered screenshot -- it checks the optics, the STEP glass
alignment, the Machine Vision menu registration, and that the operational
defaults still inherit from the 150 mm measured preset.

Run:
    .devenv/state/venv/bin/python -m KrakenOS.UI.validate_machine_vision_pyrite_120_05x_surrogate
...
def _extract_step_glass_fits(step_path: Path):
    ...
    for group in _group_candidates(candidates, aperture_diameter):
        ...
        if (
            ...
            and fit.diameter_mm < 35.0
            and fit.rms_error_mm < 0.02
        ):
            fits.append(fit)
...
def main() -> int:
    module = importlib.import_module(
        "KrakenOS.common_optical_layouts.machine_vision_120mm_pyrite_datasheet_05x"
    )
    ...
    if failed:
        print("PYRITE 120 mm 0.5X surrogate validation failed:")
        for name in failed:
            print(f"- {name}")
        ...
        return 1
    print(
        "PYRITE 120 mm 0.5X surrogate validation passed: "
