...
.. list-table::
   ...
   * - Beam Splitter Fresnel Polarization Example
     ...
     - ``KrakenOS/Examples/Examp_Beam_Splitter_Fresnel_Polarization.py`` uses ``split_mode = "Deterministic Fresnel P/S"`` and compares pure P, equal P/S, and pure S branch powers plus ``BRANCH_JONES_P``/``BRANCH_JONES_S`` on a finite BK7 splitter.
