Worked Exercise Solutions
=========================

This section contains ten reference-specific collections of textbook-style
exercises, a five-book set of KrakenOS cross-references, a contributor guide,
and a compact worked example.  The aim is not only to obtain an answer, but to
make each mathematical decision visible: what is known, which equation
applies, how it is rearranged, how units are handled, and how the result is
checked.

Start with :doc:`writing_a_solution` when adding an exercise.  The thin-lens
example introduces the solution format; book-specific collections have their
own parent pages.

.. note::

   Record the book title, edition, chapter, and exercise number, but write the
   problem in your own words unless the text permits reproduction.  A short
   paraphrase is enough to understand and solve the exercise.

.. toctree::
   :maxdepth: 1
   :caption: Book Collections

   understanding_lasers
   optical_alignment_methods/index
   introduction_laser_diode_pumped_solid_state_lasers/index
   photonics_essentials/index
   fundamentals_of_photonics/index
   hecht_optics_5e/index
   siegman_lasers/index
   yariv_yeh_photonics_6e/index
   introduction_matrix_methods_optics/index
   schaum_optics/index

.. toctree::
   :maxdepth: 1
   :caption: KrakenOS Textbook Cross-References

   krakenos_textbook_maps/index

.. toctree::
   :maxdepth: 1
   :caption: Contributor Guide and Example

   writing_a_solution
   thin_lens_image_distance
