...
.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_topic_1.svg
   :alt: Fourier partial sums reconstruct the ramp/step waveform and the triangular waveform from their independently calculated coefficients.
   :width: 100%
   :align: center
   :name: schaum-figure-8-1
...
**Formula reference.** Use :eq:`schaum-8-1`, its definitions, and :ref:`the topic illustration <schaum-figure-8-1>`.
...
**Check.** The minus sign in :math:`b_m=-C_m\sin\epsilon_m` comes from the plus sign inside :math:`\cos(mk_0x+\epsilon_m)`.
...
**Result.** Only even harmonics relative to period :math:`L` remain when the function also has period :math:`L/2`.
...
**Check.** At :math:`x=0` the series converges to the jump midpoint :math:`-\pi/2`, not to either one-sided value.
...
**Result.** :math:`f(y)=\pi/2-(4/\pi)(\cos y+\cos3y/9+\cos5y/25+\cdots)`.

**Check.** At :math:`y=0`, :math:`\sum_{j\ge0}(2j+1)^{-2}=\pi^2/8` makes the reconstructed value zero.
...
**Result.** The mean is :math:`H/2`; odd cosine amplitudes scale as :math:`-4H/[\pi^2(2j+1)^2]`.

**Check.** Setting :math:`H=\pi` and :math:`L=2\pi` recovers Problem 8.26 exactly.
...
**Result.** :math:`g(y)=(4/\pi)(\cos y+\cos3y/9+\cos5y/25+\cdots)`.
...
**Result.** The first cosine denominators are :math:`3,15,35,\ldots`, with a 1 Hz fundamental after rectification.

**Check.** Using :math:`|\sin2\pi t|` would instead give a 0.5 s rectified period.
...
.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_topic_2.svg
   :alt: A rectangular window transforms to a sinc; a Gaussian transforms to a Gaussian without the ideal sinc's sidelobes.
   :width: 100%
   :align: center
   :name: schaum-figure-8-2
...
**Formula reference.** Use :eq:`schaum-8-2`, its definitions, and :ref:`the topic illustration <schaum-figure-8-2>`.
...
**Result.** :math:`F(k)=2E_0L\operatorname{sinc}(kL)`.

**Check.** :math:`F(0)=2LE_0` equals the pulse area; the even real pulse must have an even real transform under either sign convention.
...
**Check.** The input is real and odd, so :math:`F(-k)=-F(k)=F(k)^*` and :math:`F(0)=0`.
...
**Result.** A central sinc lobe minus two half-weight sinc lobes centred at :math:`\pm2k_p`.

**Check.** :math:`F(0)=E_0[L-\sin(2k_pL)/(2k_p)]`, equal to the integral of the windowed sine squared.
...
**Result.** :math:`F(k)=2a/(a^2+k^2)`.

**Check.** :math:`F(0)=2/a` matches the total area, and the result is even and real.
...
**Result.** :math:`\mathcal F\{\sqrt{a/\pi}e^{-ax^2}\}=e^{-k^2/(4a)}`.

**Check.** Making the spatial Gaussian narrower by increasing :math:`a` broadens its Fourier transform, as expected from reciprocal width scaling.
...
**Result.** :math:`F(k)=1/(a+ik)^2` with the negative-exponential convention used here.

**Check.** :math:`F(0)=1/a^2=\int_0^\infty xe^{-ax}\,dx`, and :math:`F(-k)=F(k)^*`.
...
**Result.** :math:`\delta(x)\leftrightarrow1` and :math:`1\leftrightarrow2\pi\delta(k)`.

**Check.** The factor :math:`2\pi` is fixed by the chosen inverse-transform normalization.
...
.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_topic_3.svg
   :alt: Convolution adds positions and multiplies weights. Three unit impulses produce the five-weight sequence 1, 2, 3, 2, 1.
   :width: 100%
   :align: center
   :name: schaum-figure-8-3
...
**Formula reference.** Use :eq:`schaum-8-3`, its definitions, and :ref:`the topic illustration <schaum-figure-8-3>`.
...
**Result.** Multiplication in position corresponds to frequency convolution divided by :math:`2\pi`.

**Check.** The dual theorem :math:`\mathcal F\{f*h\}=FH` has no extra factor in this convention.
...
**Result.** Three spectral lines: DC weight :math:`\pi`, and weights :math:`\pi/2` at :math:`\pm2k_0`.

**Check.** Directly transforming :math:`\cos^2k_0x=(1+\cos2k_0x)/2` gives the same weights.
...
**Result.** :math:`f*h=h*f` whenever the convolution is well defined.
...
**Result.** The self-convolution is the five-line triangular weight sequence :math:`1,2,3,2,1`.

**Check.** The output total weight is :math:`9=3^2`, the product of the two input total weights.
...
**Result.** Nine output locations :math:`-6,-5,-4,-1,0,1,4,5,6` with weights :math:`1,2,1,2,4,2,1,2,1`.

**Check.** The weights sum to :math:`16=4^2`; there are no lines at :math:`\pm2` or :math:`\pm3`.

.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_42.svg
   :alt: All sixteen ordered sums of four spectral positions give nine distinct output positions with the displayed multiplicities.
   :width: 100%
   :align: center
   :name: schaum-figure-8-42
...
**Check.** The opposite-sign lobes have zero total area, so :math:`G(0)=0`.

.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_43.svg
   :alt: A rectangle convolved with an opposite-sign impulse pair becomes a bipolar rectangle with zero total area.
   :width: 100%
   :align: center
   :name: schaum-figure-8-43
...
**Result.** Three triangular components of base width :math:`2b`, with central peak twice either outer peak when they do not overlap.

**Check.** Their total area is :math:`4b^2=(2b)^2`, as required by the convolution area rule.

.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_44.svg
   :alt: Self-convolution of two finite slits creates three triangular components, including two coincident cross terms at the centre.
   :width: 100%
   :align: center
   :name: schaum-figure-8-44
...
**Result.** A trapezoid supported on :math:`[4,7]`, with a flat top on :math:`[5,6]` and peak :math:`AB`.

**Check.** Its area is :math:`2AB=(A\cdot1)(B\cdot2)`, matching the product of rectangle areas.

.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_45.svg
   :alt: The overlap length of unequal rectangles grows, plateaus, then decreases, producing a trapezoid rather than a triangle.
   :width: 100%
   :align: center
   :name: schaum-figure-8-45
...
**Result.** The result is the piecewise-linear interpolation of the six impulse weights, with zero endpoints at :math:`-1` and :math:`6`.

**Check.** The triangle area is one, so the output area equals the input weight sum :math:`10`.

.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_46.svg
   :alt: Shifted weighted triangular kernels add to the piecewise-linear interpolation of the impulse weights.
   :width: 100%
   :align: center
   :name: schaum-figure-8-46
...
**Result.** Nineteen hexagonally arranged convolution spots, with multiplicities :math:`6` at the centre, :math:`2` on each of two inner rings, and :math:`1` on the outer ring.

**Check.** The multiplicities total :math:`6+6(2)+6(2)+6(1)=36`, accounting for every ordered pair of the six holes.

.. figure:: /_static/knowledge_base/worked_exercises/schaum_optics/schaum_8_47.svg
   :alt: The six-hole mask has nineteen distinct pair-sum centres. Labels give convolution multiplicities, not physical hole diameters.
   :width: 100%
   :align: center
   :name: schaum-figure-8-47
