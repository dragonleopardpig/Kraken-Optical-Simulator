...
| **Left-drag** | **Orbits the camera** — everywhere (the most frequent gesture, bugs/0622). |
| **Ctrl+left-drag** | **Box-selects**: draws the selection rectangle from anywhere; release selects everything inside — then right-click for the Selection menu. Armed pick modes (measure, snap-to-axis, …) keep Ctrl-orbit instead. Shift+drag / middle-drag pans. |
