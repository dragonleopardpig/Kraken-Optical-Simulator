...
def validate_scene_sources() -> list[SceneSourceCheck]:
    ...
    checks.append(
        SceneSourceCheck(
            ...
            and np.isclose(float(np.min(azimuthal_cone_angles)), 0.0, atol=1e-9)
    ...
    checks.append(
        SceneSourceCheck(
            "non-sequential Pupil/field source cone does not auto-launch a physical point cone after scene promotion",
            len(getattr(nonseq_cone_rays, "CC", [])) >= 5
            and float(nonseq_cone_origin_span[0]) > 1e-9
            and float(nonseq_cone_origin_span[1]) > 1e-9
            and np.allclose(nonseq_cone_direction_span, 0.0, atol=1e-9),
            (
