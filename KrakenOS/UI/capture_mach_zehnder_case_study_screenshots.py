"""Capture UI screenshots for the Mach-Zehnder beam-splitter case study.
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "docs" / "source" / "_static" / "tutorials" / "mach_zehnder_interferometer"
LAYOUT_NAME = "Mach-Zehnder Interferometer (Interferogram)"
CROSS_PATH_VIEW = "Path 4: BS2 to cross output detector"
RETURN_PATH_VIEW = "Path 5: BS2 to return output detector"
...
def _capture_window_image(widget) -> Image.Image:
    tmp_path = Path("/tmp/kraken_mach_zehnder_case_capture_tmp.png")
...
def _set_dense_detector_demo(app: KrakenLayoutEditor, *, output_filter: str) -> None:
    ...
    app.branch_field_propagation_mm_var.set("0.0")
    app.arm_view_var.set("All paths")
    app.analysis_branch_filter_var.set(output_filter)
...
def _assert_analysis_filter(app: KrakenLayoutEditor, analysis_filter: str) -> None:
    choices = list(app.analysis_branch_filter_menu["values"])
    if analysis_filter not in choices:
        raise RuntimeError(f"{analysis_filter!r} was not discovered; choices={choices!r}")
...
def capture(output_dir: Path) -> list[Path]:
    ...
    try:
        ...
        outputs.append(_save_window(app, output_dir, "01_loaded_mach_zehnder_ui.png"))
        outputs.append(_save_canvas(app, output_dir, "02_mach_zehnder_path_labels.png"))

        _set_sidebars(app, left=False, right=False)
        _set_path_view(app, CROSS_PATH_VIEW)
        outputs.append(_save_window(app, output_dir, "03_cross_output_path_view_ui.png"))

        _set_path_view(app, RETURN_PATH_VIEW)
        outputs.append(_save_window(app, output_dir, "04_return_output_path_view_ui.png"))

        _set_sidebars(app, left=False, right=False)
        _set_dense_detector_demo(app, output_filter=CROSS_OUTPUT_FILTER)
        _assert_analysis_filter(app, CROSS_OUTPUT_FILTER)
        _show_state(app, analysis_mode="detector_map")
        outputs.append(_save_analysis_aoi(app, output_dir, "05_cross_detector_map_aoi.png"))

        _show_state(app, analysis_mode="coherent_detector")
        outputs.append(_save_analysis_aoi(app, output_dir, "06_cross_coherent_detector_aoi.png"))

        _show_state(app, analysis_mode="interferogram")
        outputs.append(_save_analysis_aoi(app, output_dir, "07_cross_interferogram_aoi.png"))

        _show_state(app, analysis_mode="branch_field")
        outputs.append(_save_analysis_aoi(app, output_dir, "08_cross_branch_field_aoi.png"))

        _set_dense_detector_demo(app, output_filter=RETURN_OUTPUT_FILTER)
        _assert_analysis_filter(app, RETURN_OUTPUT_FILTER)
        _show_state(app, analysis_mode="coherent_detector")
        outputs.append(_save_analysis_aoi(app, output_dir, "09_return_coherent_detector_aoi.png"))
    finally:
        app.destroy()
    return outputs
