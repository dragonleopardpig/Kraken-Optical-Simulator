...
class MainOpticalSolidFaceRolesDialog:
    ...
    def _open_optical_solid_faces_for_row(self, row_index: int, row: Any, path: Path) -> None:
        ...
        self._add_widget_tooltip(function_menu, 'Surface coating/interaction model for the selected CAD/STL face. Choose Uncoated for normal Snell-Fresnel refraction; TIR then occurs automatically when geometry and index demand it. Diffuse / Scatter Object marks the face as a diffuse scatterer (default Lambertian) so the non-sequential trace spawns scatter rays off it, like a Diffuse Object surface (bugs/0271).')
