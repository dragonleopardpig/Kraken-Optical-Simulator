...
class Open3DTopControlsPanel:
    ...
    def build_view_toolbar(self, parent: tk.Widget) -> ttk.Frame:
        ...
        add_menu_checkbuttons(
            ...
            (
                ...
                # bugs/0542: ALSO the master switch for illumination-role sources in the
                # interactive preview -- toggling must invalidate + retrace, not just re-render.
                MenuCheckbutton("Illum rays", self.inspector.show_source_illumination_rays_var, self.inspector._on_illumination_rays_toggled),
    ...
    def build_scene_toolbar(self, parent: tk.Widget) -> ttk.Frame:
        ...
        add_menu_commands(
            ...
            (
                ...
                MenuCommand("Select Elements (Rubber Band)", self.inspector.start_rubber_band_select),
                MenuCommand("Rubber-Band Select + Snap to Axis...", self.inspector.start_rubber_band_select_and_snap),
                MenuCommand("Snap Selected to Optical Axis", self.inspector.start_snap_selected_to_axis),
                MenuCommand("Add Selected to Assembly", self.inspector.group_selected_as_assembly),
                MenuCommand("Snap Assembly to Optical Axis", self.inspector.start_snap_assembly_to_axis),
                MenuCommand("Clear Assembly", self.inspector.clear_assembly),
            ),
        )
    ...
    def build_carry_toolbar(self, parent: tk.Widget) -> ttk.Frame:
        ...
        ttk.Label(carry_toolbar, text="Hold-drag STEP to move freely; drag rotates view; Ctrl+drag box-selects.").pack(side="left")
        pack_commit_checkbutton(
            carry_toolbar,
