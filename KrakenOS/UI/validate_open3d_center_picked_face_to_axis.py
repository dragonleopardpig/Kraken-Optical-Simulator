"""Guard for bugs/0119 -- the right-click face menu must offer a TRANSLATE-ONLY
"Center Picked Face -> Optical Axis", not the normal-aligning snap that rotates the
body.

Regression context
------------------
"Snap Picked Face -> Optical Axis" is a normal-align command: it rotates the STEP so
the picked face's normal is anti-parallel to the optical axis, then translates. A
user who right-clicked a window face and reached for "the axis one" wanting to
*center* it got an unwanted tilt instead ("I want to center this window to the
optical axis" -> "after Snap Picked Face -> Optical Axis, it is not what I wanted").

Fix: the right-click item is now the translate-only
`ScenePlacementMixin.center_step_feature_on_optical_axis`, wired through
`Open3DFaceAssignmentService._center_step_face_to_optical_axis_from_context`. The
normal snap stays in the top STEP menu.
...
class _FakeEditor:
    ...
    The nearest-axis frame mirrors the rays-off production case: the nearest point on
    the global +Z axis to the face centre, i.e. (0, 0, z) -- so a correct centre
    zeroes x/y and preserves z. `_set_step_rotation_deg_tuple` is present and records
    any call, so the behavioural check proves the centre is translate-only."""
