            ...
            if self._picker is not None and self._renderer is not None and self._vtk_interactor is not None:
                try:
                    ...
                    if placement_move is not None:
                        ...
                        return
                    self._set_rotation_handle_hover(None)
                    # bugs/0225 (rev 2, flag_20260705_131522): the Pick-rays HOVER belongs
                    # HERE -- the PASSIVE (idle) hover path. The first cut lived after the
                    # hover_default pick, which only runs during STEP-placement flows
                    # (target_label/axis-pick), so plain idle hovering never highlighted a
                    # ray. The passive path deliberately runs no full-scene pick, so the
                    # ray is resolved by a pick-list-restricted pick over the ~8 merged
                    # ray actors (cheap), gated on the Pick-rays toggle.
                    hovered_ray = None
                    try:
                        if self._ray_pick_enabled():
                            hovered_ray = self._passive_hover_pick_ray(x, y)
                    except Exception:
                        hovered_ray = None
                    ...
                    if hover_overlay_changed:
                        self.render()  # un-hovered a ray: drop the overlay, keep the normal hover flow
                    step_label = self._actor_step_map.get(actor_key) if actor_key is not None else None
                    fallback_step_pick = None
                    if step_label is None:
