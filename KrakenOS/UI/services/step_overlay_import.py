...
class StepOverlayImportService:
    ...
    def import_optical_step(
        ...
        *,
        ...
        # Carry-first: do NOT auto-promote or auto-snap to the optical
        # axis on import. The overlay arrives in carry mode with rotation
        # handles (the inspector's import handler starts the carry follow),
        # so the user corrects orientation first. Promoting to analytic
        # surfaces -- which snaps the body onto the optical axis -- is an
        # explicit later action ("Promote STEP to Analytic Surfaces"); it
        # offers the same glass-sequence prompt, pre-filled from a Zemax
