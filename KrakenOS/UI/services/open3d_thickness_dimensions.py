...
class Open3DThicknessDimensionService:
    ...
    def _emit_led_object_edge_dimension(
        ...
        *,
        ...
        bugs/0132: a right-click -> "Re-anchor to a surface/edge..." (or a drag of this
        arrow) re-points it to a picked LED face/edge and makes that face the LED's OWN
        object-edge reference (``apply_led_object_edge_reanchor`` sets
        ``led_step_object_edge_local_z`` + the typed distance), so the body does not jump
        now but a later edge-distance edit MOVES the LED with the arrow on the chosen
        face. The arrow registers a drag record so the re-anchor handle exists (a
        value-drag is harmless -- the sentinel row is rejected by
        ``drag_state_from_current_pick``). Reverses bugs/0130's measurement-only override
        (which reverted the arrow to the typed cable extremum on value-change)."""
        editor = self.editor
        ...
        # bugs/0132: re-anchoring this arrow onto a picked LED face sets the LED's OWN
        # object-edge reference (led_step_object_edge_local_z) and the typed distance, so
        # the chosen face IS what ``distance`` measures to -- there is no measurement-only
        # override to honour here. bugs/0125: a free axial carry-drag adds
        # placement_offset_z on top WITHOUT updating the typed value, so measure to the
        # live object-side edge (== ``distance`` when undragged).
