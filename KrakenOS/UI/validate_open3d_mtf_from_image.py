...
def _check_contract(failures, notes):
    ...
    for token, msg in (
        ("analyze_usaf_image", "the dialog does not call analyze_usaf_image"),
    ...
    if not [f for f in failures if f.startswith("CONTRACT")]:
        notes.append("contract = draw ROIs (press/motion/release) in image px, analyze, plot, save CSV")


def run_checks() -> "tuple[bool, list[str]]":
    failures: list[str] = []
    notes: list[str] = []
    for check in (_check_analyze, _check_wiring, _check_contract):
        try:
